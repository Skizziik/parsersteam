...
class SteamReviewParser:
    ...
    def get_reviews(self, cursor="*", num_per_page=100):
        params = {
            ...
            "cursor": cursor,
            "purchase_type": "all",
            "review_type": "negative"  # ТОЛЬКО НЕГАТИВНЫЕ ОТЗЫВЫ
        }
    ...
    def parse_all_negative_reviews(self, max_batches=None):
        ...
        if max_batches:
            print(f"Максимум партий: {max_batches}")
        print("-" * 60)
        ...
        pbar = tqdm(
            ...
            unit=" отзывов",
            ncols=100,
            bar_format='{desc}: {n_fmt} [{bar}] | Просмотрено: {postfix[0]} | В БД: {postfix[1]}',
        ...
        try:
            while True:
                if max_batches and batch_num > max_batches:
                    print(f"\nДостигнут лимит партий ({max_batches})")
                    break
                ...
                if not reviews:
                    ...
                    if empty_batches > 3:
                        ...
                        break
                    print("Пустая партия, пробуем еще...")
                    time.sleep(5)
                    continue
                ...
                pbar.n = current_db_count
                pbar.refresh()
                
                new_cursor = data.get('cursor', None)
                
                if not new_cursor or new_cursor == cursor:
                    print("Курсор не изменился, достигнут конец")
                    break
...
if __name__ == "__main__":
    FM2024_APP_ID = "2252570"  # <-- ИЗМЕНИТЕ ЗДЕСЬ НА ID НУЖНОЙ ИГРЫ
    
    parser = SteamReviewParser(FM2024_APP_ID)
    
    # Попробуем собрать больше отзывов
    print("\n" + "="*60)
    print("ВНИМАНИЕ: Steam ограничивает количество отзывов")
    print("Парсер будет пытаться собрать максимум возможных")
    print("Если остановится - запустите снова через некоторое время")
    print("="*60)
